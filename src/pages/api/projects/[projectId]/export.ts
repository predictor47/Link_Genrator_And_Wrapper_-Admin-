...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const surveyLinksResult = await amplifyServerService.listSurveyLinksByProject(projectId);
    ...
    const exportData = paginatedLinks.map((link: SurveyLink) => {
      ...
      return {
        id: link.id,
        uid: link.uid,
        wrappedUrl: `${req.headers.host?.includes('localhost') ? 'http://' : 'https://'}${req.headers.host}/s/${projectId}/${link.uid}`,
        originalUrl,
